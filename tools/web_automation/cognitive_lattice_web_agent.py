#!/usr/bin/env python3
"""
Enhanced Cognitive Lattice Web Agent
===================================

Implements the full cognitive lattice pattern:
1. Create plan before navigation
2. Execute step-by-step with DOM analysis after each action
3. Update lattice with progress and DOM changes
4. Continuously adapt based on what's actually on the page
"""
...
from tools.web_automation.vision_dom_reasoner import (
...
INTERACTIVE_MAX_ITEMS = 100  # Increased to give AI more choices
SELECTED_CLASS_TOKENS = {"selected","active","is-selected","is-active","chosen","checked"}

def extract_selection_signals(html: str) -> list[dict]:
    # Move the selection signals logic from ai_web_agent_tool.py
    signals = []
    for m in re.finditer(r"<([a-zA-Z0-9]+)\b([^>]*)>", html or ""):
        ...
        aria_sel = (attrs.get("aria-selected") or "").lower() == "true"
        aria_pressed = (attrs.get("aria-pressed") or "").lower() == "true"
        aria_checked = (attrs.get("aria-checked") or "").lower() == "true"
        data_selected = "data-selected" in attrs
        has_sel_class = any(tok in cls for tok in SELECTED_CLASS_TOKENS)

        if aria_sel or aria_pressed or aria_checked or data_selected or has_sel_class:
            label = attrs.get("aria-label") or attrs.get("name") or ""
            signals.append({
                "tag": tag,
                "role": attrs.get("role",""),
                "aria_label": label,
                ...
                "class": cls,
                "selected_flags": {
                    "aria_selected": aria_sel,
                    "aria_pressed": aria_pressed,
                    "aria_checked": aria_checked,
                    "data_selected": data_selected,
                    "class_selected": has_sel_class,
                }
            })
    return signals

class CognitiveLatticeWebAgent:
    """
    Enhanced web agent with cognitive lattice integration and continuous DOM monitoring.
    """
    
    def __init__(self, external_client=None, cognitive_lattice=None, enable_stealth=True):
        self.external_client = external_client
        self.lattice = cognitive_lattice  # Real CognitiveLattice instance
        
        # Create shared browser instance with stealth enabled by default
        self.browser = BrowserEngineTool(enable_stealth=enable_stealth)
        self.memory = {}  # Merged memory from ai_web_agent_tool
        
        self.session_log = []  # Track all actions and DOM changes
        self.dom_history = []  # Track DOM changes after each action
        self.current_web_task = None  # Current active web task
        
    def _build_lattice_context(self) -> Dict[str, Any]:
        ...
        if len(failed_actions) >= 2:
            insights.append("Recent failures detected - review element selection strategy")
        
        # Pattern: DOM changes without achieving goal
        dom_changes_without_success = [action for action in recent_actions 
                                     if action.get('dom_changes', {}).get('changes_detected', False) 
                                     and not action.get('action_result', {}).get('achieved', False)]
        if len(dom_changes_without_success) >= 2:
            insights.append("DOM changes occurring but goals not achieved - may need different approach")
        
        # Pattern: Successful action types
        successful_actions_list = [action for action in recent_actions 
                                 if action.get('action_result', {}).get('achieved', False)]
        if successful_actions_list:
            insights.append(f"Recent successful pattern: {len(successful_actions_list)} successful actions")
        
        # Build session context from active task
        session_context = {}
        if active_task:
            session_context = {
                'goal': active_task.get('query', 'No active goal'),
                'task_title': active_task.get('task_title', 'No active task'),
        ...
        return {
            ...
            'insights': insights,
            'total_steps': len(recent_actions)
        }
    ...
    async def observe(self, goal: str = "") -> Dict[str, Any]:
        """Enhanced observation with interactive elements and semantic memory, goal-aware DOM compression"""
        ...
        interactive = summarize_interactive_elements(raw_dom, max_items=INTERACTIVE_MAX_ITEMS)
        ...
        obs = {
            ...
            "raw_dom": raw_dom,  # Add raw DOM for skeleton processing
            "compressed_dom": compressed,
            "interactive_summary": interactive,
            "selection_state": selection[:20],
            "semantic_memory": self.memory.get("semantic_labels", {}),
            "domain_memory": domain_memory,  # Add persistent domain memory
        }

        # Add recent DOM change notes
        if self.memory.get("dom_change_history"):
            obs["dom_change_history"] = "\n".join(self.memory["dom_change_history"][-3:])
    ...
    def update_lattice_step(self, step_description: str, action_result: Dict[str, Any], dom_changes: Dict[str, Any]) -> None:
        ...
        # Create comprehensive step result for lattice
        step_result = {
        ...
        if active_task and step_success:
            ...
            self.lattice.execute_step(
                ...
                result=f"Web step completed: {step_success}"
            )
        ...
        if self.external_client:
            try:
                ...
                if json_match:
                    ...
                    if self.lattice:
                        task_id = self.lattice.create_new_task(
                            query=high_level_goal,
                            task_plan=plan.get('execution_plan', [])
                        )
                        self.current_web_task = task_id
                        print(f"[LATTICE] Created new task from dynamic plan: {task_id}")
...
"""
    ...
    async def execute_plan_with_monitoring(self, execution_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        print(f"🚀 Starting plan execution: {execution_plan.get('plan', {}).get('goal', 'Unknown goal')}")
        
        # Get current lattice context
        step_context = self.get_current_step_context()
        
        # Initialize browser ONCE if not already initialized
        await self.ensure_browser(headless=False)
        
        # Navigate to target URL if we're starting fresh
        target_url = execution_plan.get('plan', {}).get('target_url')
        if target_url and step_context.get("current_step_index", 0) == 0:
            ...
            self.update_lattice_step(
                step_description="Navigate to target website",
                action_result={"achieved": True, "current_url": target_url},
                dom_changes={"changes_detected": True, "new_elements": ["page loaded"]}
            )
        
        # Get initial DOM state
        initial_dom = await self.browser.page.content()
        self.dom_history.append({
            "step": 0,
            "action": "initial_load",
            "dom_hash": hash_dom(initial_dom),
            "timestamp": datetime.now().isoformat()
        })
        
        # Execute each step in the plan
        plan_steps = execution_plan.get('plan', {}).get('execution_plan', [])
        results = []
        
        # Start from where we left off based on lattice context
        start_step = step_context.get("current_step_index", 0)
        
        for step_index, step in enumerate(plan_steps):
            if step_index < start_step:
                print(f"⏭️ Skipping step {step.get('step_id')} - already completed in lattice")
                continue
                
            print(f"\n🎯 Step {step.get('step_id', step_index + 1)}: {step.get('goal', 'Unknown step')}")
            
            # Check lattice context for this step - has it been done?
            if step.get('goal') in step_context.get("completed_actions", []):
                print(f"✅ Step already completed according to lattice: {step.get('goal')}")
                continue
            
            # Get observation for this step using our own observe method with goal context
            step_goal = step.get('goal', '')
            obs = await self.observe(step_goal)
            
            # Add execution plan context to observation, including ZIP code extraction
            overall_goal = execution_plan.get('plan', {}).get('goal', '')
            zip_code_match = re.search(r'\b(\d{5})\b', overall_goal)
            extracted_zip = zip_code_match.group(1) if zip_code_match else None
            
            obs['execution_plan_context'] = {
                'overall_goal': overall_goal,
                'extracted_zip_code': extracted_zip,
                'current_step': step.get('step_id', step_index + 1),
                'total_steps': len(plan_steps),
                'step_goal': step.get('goal', '')
            }
            
            # Build comprehensive lattice context for epistemic self-awareness
            lattice_context = self._build_lattice_context()
            obs['lattice_context'] = lattice_context
            
            # Simple step goal without lattice append (lattice context now in obs)
            step_goal = f"{step.get('goal', '')} (Step {step.get('step_id', step_index + 1)} of {len(plan_steps)})"
            
            # Get AI reasoning for this specific step
            reasoning = llm_reason(step_goal, obs, external_client=self.external_client)
            actions = reasoning.get("actions", [])
            
            if not actions:
                print(f"⚠️ No actions generated for step {step.get('step_id')}")
                # Update lattice with no-action step (not successful)
                self.update_lattice_step(
                    step_description=step.get('goal', f'Step {step.get("step_id")}'),
                    action_result={"achieved": False, "reason": "no actions generated"},
                    dom_changes={"changes_detected": False}
                )
                continue
            
            # Execute actions using our own execute_actions method
            action_results = await self.execute_actions(actions)
            
            # ENHANCED DOM ANALYSIS using DOM diffing
            dom_changes_detected = any(
                result.get("dom_before_hash") != result.get("dom_after_hash") 
                for result in action_results if result.get("dom_before_hash") and result.get("dom_after_hash")
            )
            
            # Aggregate change analysis from all actions
            aggregated_change_analysis = {
                "has_changes": False,
            ...
            for result in action_results:
                ...
                if change_analysis.get("has_changes"):
                    aggregated_change_analysis["has_changes"] = True
                    aggregated_change_analysis["new_interactive_elements"].extend(
                        change_analysis.get("new_interactive_elements", [])
                    )
                    aggregated_change_analysis["content_patterns"].extend(
                        change_analysis.get("content_patterns", [])
                    )
                    # Use the most detailed change summary
                    if change_analysis.get("change_summary", "") != "No changes detected":
                        aggregated_change_analysis["change_summary"] = change_analysis.get("change_summary", "")
            
            # Enhanced DOM analysis with pattern recognition
            dom_analysis = {
                "changes_detected": dom_changes_detected,
                "change_type": _classify_change_type(aggregated_change_analysis, action_results),
                "new_elements": aggregated_change_analysis["new_interactive_elements"],
                "content_patterns": aggregated_change_analysis["content_patterns"],
                "dom_summary": aggregated_change_analysis["change_summary"],
                "location_results_appeared": "location_results" in aggregated_change_analysis["content_patterns"]
            }
            
            # CONTEXT-AWARE VERIFICATION: Use DOM diff for granular changes
            step_goal_text = step.get('goal', '').lower()
            action_type = actions[0].get('type', '') if actions else ''
            use_diff = should_use_dom_diff(action_type, step_goal_text)
            
            print(f"🔍 DOM Diff Decision: use_diff={use_diff}, action_type='{action_type}', goal='{step_goal_text}'")
            
            if use_diff and aggregated_change_analysis["has_changes"]:
                # Use focused DOM diff for verification instead of full DOM
                new_elements_html = ""
                for result in action_results:
                    change_analysis = result.get("change_analysis", {})
                    if change_analysis.get("new_elements_html"):
                        new_elements_html += change_analysis["new_elements_html"] + "\n"
                
                # CRITICAL FIX: Clean the DOM diff content before sending to API
                cleaned_dom_diff = compress_dom(new_elements_html.strip(), step_goal_text)
                
                # DEBUG: Save both raw and cleaned DOM diff being sent to API
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                dom_diff_file = f"debug_dom_diff_sent_to_api_{timestamp}.txt"
                with open(dom_diff_file, 'w', encoding='utf-8') as f:
                    f.write(f"DOM DIFF SENT TO API at {datetime.now()}\n")
                    f.write(f"Goal: {step_goal_text}\n")
                    f.write(f"Action Type: {action_type}\n")
                    f.write(f"Use Diff Decision: {use_diff}\n")
                    f.write(f"Raw DOM Diff Length: {len(new_elements_html.strip())} characters\n")
                    f.write(f"Cleaned DOM Diff Length: {len(cleaned_dom_diff)} characters\n")
                    f.write("=" * 80 + "\n")
                    f.write("RAW DOM DIFF CONTENT:\n")
                    f.write(new_elements_html.strip()[:2000])  # First 2000 chars of raw
                    f.write("\n" + "=" * 40 + " CLEANED " + "=" * 40 + "\n")
                    f.write("CLEANED DOM DIFF CONTENT (SENT TO API):\n")
                    f.write(cleaned_dom_diff)
                    f.write("\n" + "=" * 80 + "\n")
                    f.write("AGGREGATED CHANGE ANALYSIS:\n")
                    f.write(json.dumps(aggregated_change_analysis, indent=2))
                
                print(f"🔍 DEBUG: Saved DOM diff (raw: {len(new_elements_html.strip())}, cleaned: {len(cleaned_dom_diff)} chars) to {dom_diff_file}")
                print(f"🎯 Using cleaned DOM diff for focused verification (step: {step_goal_text})")
                verification = llm_verify(step.get('goal', ''), action_results, cleaned_dom_diff, external_client=self.external_client)
            else:
                # Use full DOM for major changes, navigation, or location selection
                post_action_dom = await self.browser.page.content()
                compressed_after = compress_dom(post_action_dom)
                print(f"🌐 Using full DOM for verification (step: {step_goal_text})")
                verification = llm_verify(step.get('goal', ''), action_results, compressed_after, external_client=self.external_client)

            # Enhanced rule-based fallback verification for common cases
            goal_text = (step.get('goal') or '').lower()
            url_now = (await self.browser.get_page_info()).get("url", "")
            rule_achieved = False
            
            # Get previous URL for comparison (fix for undefined signals variable)
            previous_url = getattr(self, '_previous_url', '')
            
            # Enhanced signals including DOM change detection
            signals = {
                "url": url_now,
                "url_changed": url_now != previous_url,
                "url_fragment_only": (previous_url.split('#')[0] == url_now.split('#')[0] and previous_url != url_now),
                "change_type": dom_analysis.get('change_type'),
                "new_interactives_count": len(dom_analysis.get('new_elements', [])),
                "dom_changed": dom_changes_detected,
                "action_succeeded": all(r.get("exec_status") == "ok" for r in action_results),
                "element_appeared": len(dom_analysis.get('new_elements', [])) > 0,
                "modal_detected": dom_analysis.get('change_type') == 'modal_opened'
            }
            
            # Store current URL for next iteration
            self._previous_url = url_now
            
            # More sophisticated rule checking based on goal type
            # PRIORITY: Check location-specific signals FIRST (addresses, search results)
            last_action_type = action_results[0].get('type') if action_results else 'noop'
            
            # Fragment-only changes are OK if DOM also changed (valid anchor links)
            fragment_only_no_dom = signals.get("url_fragment_only", False) and not dom_changes_detected
            real_change_detected = dom_changes_detected and not fragment_only_no_dom
            
            # LOCATION SEARCH: Check for location-specific success signals FIRST
            location_success = False
            if any(k in goal_text for k in ["location", "zip", "address", "search", "find"]):
                # Generic location search success indicators
                search_signals = [
                    ...
                    "button:has-text('Pick')", "a:has-text('Select')",
                    
                    # Generic location indicators
                    "[data-testid*='location']", "[data-testid*='store']", 
                    "[data-testid*='address']", "[data-testid*='result']",
                    
                    # Distance/location text patterns (generic)
                    "div:has-text('mile')", "div:has-text('mi')", "div:has-text('km')",
                    "div:has-text('distance')", "*:has-text('miles away')",
                    
                    # Generic result containers
                    ".result", ".location", ".store", ".address",
                    "[class*='result']", "[class*='location']", "[class*='store']",
                    
                    # Common location metadata
                    "[role='definition']" # Could contain addresses/descriptions
                ]
                try:
                    for sel_sig in search_signals:
                        if await self.browser.page.query_selector(sel_sig):
                            location_success = True
                            print(f"🎯 Location search success signal detected: {sel_sig}")
                            break
                except Exception:
                    pass
            
            # GENERAL RULE: For click/type steps, check DOM changes OR location success
            if last_action_type in ("click", "type"):
                if location_success:
                    rule_achieved = True
                    print(f"✅ Location search success: Addresses/options appeared after {last_action_type}")
                elif not real_change_detected:
                    rule_achieved = False
                    if fragment_only_no_dom:
                        print(f"🚫 Rule failed: {last_action_type} action produced fragment-only URL change with no DOM change (intercepted click)")
                    else:
                        print(f"🚫 Rule failed: {last_action_type} action produced no real DOM change")
                else:
                    rule_achieved = True
            else:
                # Additional success signals for non-click/type actions
                if any(k in goal_text for k in ["order", "pickup", "delivery"]):
                    rule_achieved = real_change_detected or signals["modal_detected"] or signals["element_appeared"]
                elif any(k in goal_text for k in ["accept", "dismiss", "cookie"]):
                    rule_achieved = real_change_detected or signals.get("clicked_element_disappeared", False)
                else:
                    rule_achieved = real_change_detected or location_success

            # Combine with LLM verification, but give precedence to rules
            llm_ok = verification.get("achieved", False)
            verified = llm_ok and rule_achieved if last_action_type in ("click", "type") else (llm_ok or rule_achieved)

            # For location searches, override strict verification if location success was detected
            if location_success and last_action_type in ("click", "type"):
                verified = True
                print("✅ Location search override: Addresses appeared, considering step successful")
            ...
            self.session_log.append({"type": "step_verification", "data": step_result})
            print(f"📊 Step {step_index+1} Results:\n   Verified: {verified}\n   Changes detected: {dom_changes_detected}\n   Change type: {dom_analysis.get('change_type')}\n   Recommendation: {'Continue' if verified else 'Retry or adjust'}")
        
        return {
            "plan_goal": execution_plan.get('goal'),
            "steps_executed": len(results),
            "results": results,
            "dom_history": self.dom_history,
            "final_state": await self.observe()
        }
...
# Convenience function for cognitive lattice integration
async def execute_cognitive_web_task(goal: str, url: str, external_client=None, cognitive_lattice=None) -> Dict[str, Any]:
    """
    Execute a complete web automation task with cognitive lattice integration.
    
    This implements the full pattern:
    1. Create plan before navigation
    2. Execute with continuous DOM monitoring
    3. Update lattice with progress
    """
    agent = CognitiveLatticeWebAgent(external_client=external_client, cognitive_lattice=cognitive_lattice)
    
    try:
        # Phase 1: Create execution plan (before going to website)
        print("📋 Phase 1: Creating execution plan...")
        execution_plan = await agent.create_execution_plan(goal, url)
        
        # Phase 2: Execute plan with monitoring (go to website and execute)
        print("🚀 Phase 2: Executing plan with DOM monitoring...")
        results = await agent.execute_plan_with_monitoring(execution_plan)
        
        return {
            "success": True,
            "execution_plan": execution_plan,
            "results": results,
            "dom_changes": len(agent.dom_history),
            "message": "Cognitive web task completed"
        }
        ...
        return {
            "success": False,
            "error": str(e),
            "partial_results": getattr(agent, 'session_log', [])
        }
    finally:
